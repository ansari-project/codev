...
Update status by modifying `codev/builders.md`:
```markdown
...
Or use the CLI:
```bash
# From within the builder worktree
agent-farm status set implementing "writing tests"
agent-farm status set blocked "Need clarification on API format"
agent-farm status set pr-ready
```
